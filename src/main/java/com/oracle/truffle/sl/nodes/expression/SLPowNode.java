...
/**
 * This class is similar to the extensively documented {@link SLMulNode}.
 */
@NodeInfo(shortName = "^")
...
//            res = ExactMath.multiplyExact(res, left);
//        }
//        return res;
        ...
        while (true) {
            if ((right & 1L) != 0L) {
                y = ExactMath.multiplyExact(p, y);
            }
            right = right >> 1;
            if (right == 0) {
                return y;
            }
            p = ExactMath.multiplyExact(p, p);
        }
    }
