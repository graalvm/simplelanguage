...
MAIN_CLASS="com.oracle.truffle.tcl.launcher.TclMain"
...
if [[ "$GRAALVM_VERSION" != "" ]]; then
    ...
    LAUNCHER_PATH="$SCRIPT_HOME/../jre/languages/tcl/launcher/tcl-launcher.jar:$SCRIPT_HOME/../languages/tcl/launcher/tcl-launcher.jar"
    JAVACMD="$SCRIPT_HOME/java"
    versionCheck "Installed in wrong version of GraalVM. Expected: $VERSION, found $GRAALVM_VERSION"
else
    LANGUAGE_PATH="$SCRIPT_HOME/language/target/tcl.jar"
